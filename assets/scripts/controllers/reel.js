...
cc.Class({
    ...
    onLoad: function () {
        ...
        var firstStopPrefab = this.stops[0];
        if (!firstStopPrefab) {
            console.error("No stop prefab defined in the 'stops' array.");
            return;
        }

        var firstStop = cc.instantiate(firstStopPrefab);
        if (!firstStop) {
            console.error("Failed to instantiate the first stop prefab.");
            return;
        }
